...
{
    public class BirthdateAnswerer
    {
        ...
        public BirthdateAnswerer(ILogger logger)
        {
            _logger = logger;
        ...
        public BirthdateAnswer GetBirthdate(string personName)
        {
            ...
            foreach (Doc doc in searchResult.Results)
            {
                ...
                
                {
                    ...
                    if (article != null)
                    {
                        ...
                        {
                            return _articleAnalyser.GetBirthDate(article);
                        }
                      ...
                      || doc.FirstTwoSentences.IndexOf(personName, StringComparison.CurrentCultureIgnoreCase) != -1)
                {
                    ...
                    var lookupResult = _provider.GetArticleAsync(articleRequest);
                    lookupResult.Start();
                    var xmlMarkupTrimmer = new XhtmlMarkupTrimmer();
                    lookupResult.Wait();
                    var article = lookupResult.Result;
                    ...
                    if (article != null
                        ...
                        && isBiography)
                    {
                        var alternativeNames = xmlMarkupTrimmer.Trim(article.Metadata.AlternativeForm).Text.Split(',');
                        foreach (var alternativeName in alternativeNames)
                        {
                            if (LevensteinDistance.Calculate(alternativeName, personName) <= 2)
                            {
                                return new ArticleAnalyser(_logger).GetBirthDate(article);   
                            }
                        }
                    }
                }
